...
Looking at the results from the benchmark, we can first see that in Jolt is objectively the fastest at proof generation across both ARM and x86 architecture, which shows it is the most capable when it comes to real-world applications. Although Risc-0 is the first production-ready zkVM, it is the worse performing at larger test cases. Additionally, RISC-0 is almost 2x faster on the ARM chip due to the fact that it is the same architecture. When looking at SP1's performance, it almost always performs between Jolt and RISC-0.

Verification time is not a bottleneck when it comes to real-world applicability. The verification benchmark gives almost a complete opposite result compared since   
...
Our work differs from related work in that we focus on a holistic comparable analysis of zkVMs for real world applications. We believe our approach is more relevant than referenced papers because us focusing on zkVMs and real world applications is the most relevant for development use cases compared to other work whose results do not give you much practical insight. Additionally, compared to publicly available benchmarks, our work is unbiased and also accompanied by a qualitative architecture comparison; publicly available benchmarks do not provide such analysis. Our approach had some limitations. First, we were limited by time constraints which made it difficult for us to benchmark more complicated ML models, as we would have had to implement them all from scratch for most of the VMs. We also did not compare memory usage which would have been an interesting metric to focus on. However, we are yet to find a publicly available benchmark that provides this comparison. Additionally, diving even deeper into the architectural design differences would have been very interesting. These improvements are all worth exploring in the near future.
