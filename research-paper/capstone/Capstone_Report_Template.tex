...
Currently, the most-friendly way of generating a ZKP is through the use of zero-knowledge virtual machines (zkVMs). A zkVM, is simply a VM implemented as a circuit for a ZKP system. So, instead of proving the execution of a program, as one would normally do in ZKP systems, you prove the execution of the bytecode of a given Instruction Set Architecture (ISA). However, the zkVM landscape is in early development, and proof generation is bottlenecked by the complexity of the program and hardware limitations. In this paper, we provide a quantatitve and qualitative analysis on the current state of zkVMs on a real-world use case by generating a proof of a machine learning algorithm on dummy financial data.

\subsection{Objective}

We propose a service that, given access to personal credit, transaction, income information, we are able to generate a ZKP\cite{Groth2016} that proves statements on their personal financial information. This way we can build a public and open ecosystem for app developers to build additional financial services wihout sacrificing privacy. To implement this, we interface user financial data by using the same API values as Plaid, a service that provides credit card history data. We generate a ZKP using the financial data as input on three zero-knowledge virtual machines (zkVMs): SP1\cite{Roy2024}, Risc0\cite{bruestle2023risc}, and Jolt\cite{arun2024jolt}.to generate proofs over this data proving statements relevant to the use cases of the data.
