...
To enhance modeling effectiveness, the dataset was processed to extract key features such as recency, frequency, monetary value (RFM), and spending activity. 
RFM serves as a foundation for predictive modeling, where these metrics can be combined with other features to enhance the accuracy of customer spending predictions.
In a real world business scenario, we can segment customers into actionable groups, such as loyal customers, recent but low-frequency buyers, or high-value customers. This segmentation aids in personalizing marketing strategies, optimizing resource allocation, and improving customer lifetime value (CLV). 
\begin{table}[ht!]
\centering
